"""Prefect flow: standalone Federal Register ingestion for isolated testing.

Ingests from the Federal Register API, extracts, processes, and loads.
...
@task(name="us-ingest")
def fetch_us(since: datetime) -> list[dict]:
    """Fetch new Federal Register documents."""
    return fr_ingest(since)


@task(name="us-extract")
...
@flow(name="ingest-us")
def ingest_us_flow(since: datetime | None = None) -> dict:
    """Standalone Federal Register ingestion flow.

    Parameters
    ----------
    since : datetime | None
        Fetch documents published after this datetime.
    ...
    try:
        ...
        logger.info("Federal Register ingest complete: %s", summary)
        return summary
